...
// Tenta remover pontos de forma segura. Retorna true se bem-sucedido.
function tryRemovePontos(userId, amount) {
        if (amount <= 0) return false;
        const data = loadPontos();
        data[userId] = data[userId] || { pontos: 0, pomodorosConcluidos: 0 };
        if ((data[userId].pontos || 0) < amount) return false;
        data[userId].pontos -= amount;
        savePontos(data);
        atualizarRankJSON();
        return true;
...
function addItemToUser(userId, itemName) {
        const data = loadPontos();
        data[userId] = data[userId] || { pontos: 0, pomodorosConcluidos: 0, itens: [] };
        if (!data[userId].itens) data[userId].itens = [];
        data[userId].itens.push(itemName);
        savePontos(data);
}
...
module.exports = { addPontos, getPontos, addPomodoro, loadPontos, savePontos, getTodosPontos, atualizarRankJSON, tryRemovePontos, addItemToUser };
